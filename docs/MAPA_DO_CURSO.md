...
```mermaid
flowchart TD
    GUIA[Guia do candidato] --> LP[Língua Portuguesa]
    GUIA --> RL[Raciocínio Lógico]

    LP --> LE[Legislação e Ética na Administração Pública]
    LP --> AG[Administração Geral e Gestão Organizacional]
    RL --> AG

    LE --> AG
    LE --> AFO[Administração Financeira e Orçamentária]
    AG --> AFO
    RL --> AFO

    LE --> RURAL[Políticas Públicas para o Desenvolvimento Rural e Agropecuário]
    AG --> RURAL
    AFO --> RURAL
